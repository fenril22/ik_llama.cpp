...
#pragma unroll
    for (int k_KQ_0 = 0; k_KQ_0 < Dk/2; k_KQ_0 += WARP_SIZE) {
        ...
        float c = PI_COS[p % 64];
        float s = PI_SIN[p % 64];
        float kx = ( c * q0 + s * q1) * norm;
        float ky = (-s * q0 + c * q1) * norm;

        sum += kx * Q_f2[k_KQ_0/WARP_SIZE].x;
        sum += ky * Q_f2[k_KQ_0/WARP_SIZE].y;
    }
...
// ── PlanarQuant3 V dequantize (per-element): inverse Givens + centroid ──
template <typename T>
static __device__ __forceinline__ T dequantize_1_planar3_0(const void * __restrict__ vx, const int64_t i) {
    ...
    // Need the pair partner for inverse Givens
    ...
    float result;
    if (j % 2 == 0) {
        // First element of pair: c*q0 + s*q1
        result = (c * q_self + s * q_partner) * norm;
    } else {
        // Second element of pair: -s*q0 + c*q1
        result = (-s * q_partner + c * q_self) * norm;
    }

    return (T)result;
}

// ── IsoQuant3 V dequantize (per-element): inverse quaternion + centroid ──
template <typename T>
...
// ── PlanarQuant4 V dequantize (per-element): inverse Givens + 4-bit centroid ──
template <typename T>
...
// ── IsoQuant4 V dequantize (per-element): inverse quaternion + 4-bit centroid ──
template <typename T>
static __device__ __forceinline__ T dequantize_1_iso4_0(const void * __restrict__ vx, const int64_t i) {
    ...
    for (int c = 0; c < 4; c++) {
        int jj = g * 4 + c;
        qvals[c] = PI_CENTROIDS_4BIT[(x[ib].qs[jj/2] >> ((jj%2)*4)) & 0xF];
    }
...
// WHT already applied to K before quantization, so just centroid lookup + norm.
#include "turbo-quant.cuh"

template <typename T, int Dk>
static __device__ __forceinline__ T vec_dot_fattn_vec_KQ_turbo3_0(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {
...
template <typename T, int Dk>
static __device__ __forceinline__ T vec_dot_fattn_vec_KQ_turbo4_0(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {
...
template <typename T, int Dk>
static __device__ __forceinline__ T vec_dot_fattn_vec_KQ_turbo2_0(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {
...
static __device__ __forceinline__ T dequantize_1_turbo3_0(const void * __restrict__ vx, const int64_t i) {
    ...
    const float norm = __half2float(x[ib].norm);
    return (T)turbo3_dequant_element(&x[ib], (int)(i % QK_TURBO3), norm);
}

template <typename T>
static __device__ __forceinline__ T dequantize_1_turbo4_0(const void * __restrict__ vx, const int64_t i) {
    const block_turbo4_0 * x = (const block_turbo4_0 *) vx;
    const int64_t ib = i / QK_TURBO4;
    const float norm = __half2float(x[ib].norm);
    return (T)turbo4_dequant_element(&x[ib], (int)(i % QK_TURBO4), norm);
}

template <typename T>
static __device__ __forceinline__ T dequantize_1_turbo2_0(const void * __restrict__ vx, const int64_t i) {
